...
import { Bell, Search, ShieldCheck, AlertCircle, LogOut, CheckCircle2, X } from 'lucide-react';
    ...
    <header className="sticky top-0 z-30 bg-white border-b border-slate-200 shrink-0">
      ...
      {environment === 'sandbox' && (
        <div className="bg-amber-50 border-b border-amber-200 px-6 py-2 flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs font-semibold text-amber-900">
            ...
            <span>SANDBOX ENVIRONMENT ACTIVE — Simulated keys (`sk_test_*`), real money moves disabled.</span>
          </div>
          <button
      ...
      <div className="h-16 px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <h2 className="text-lg font-bold text-slate-900">{activeTabTitle}</h2>
          <span className="text-xs px-2.5 py-1 rounded-md bg-slate-100 border border-slate-200 text-slate-600 font-mono">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center bg-slate-100 p-1 rounded-lg border border-slate-200">
            <button
              onClick={() => onEnvironmentToggle('live')}
              className={`px-3 py-1 text-xs font-semibold rounded-md fast-transition cursor-pointer ${
                environment === 'live'
          ...
          <div className="relative">
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-slate-200 p-4 space-y-3 z-50 animate-in fade-in zoom-in-95 duration-150">
